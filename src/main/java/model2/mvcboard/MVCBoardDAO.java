	...
	public void downcountPlus (String idx) {
		String query = "UPDATE mvcboard SET downcount = downcount + 1 WHERE idx = ?";
			...
			try {
				...
				e.printStackTrace();
				System.out.println("다운로드 증가 예외 발생");
			}
			
		}
	//비밀번호 확인 메소드(입력한 비밀 번호가 DataBase의 값과 일치하는지 확인
	public boolean confirmPassword(String pass, String idx) {
		boolean isCorr = true;
		
		try {
			//COUNT(*)가 존재하면 client에서 넘긴 값이 존재
			//COUNT(*)가 존재하지않으면 client에서 넘긴 값이 DB에 존재하지 않음
